    ...
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      ...
      {result && (
        <div className="mt-6 space-y-4">
          ...
          {/* Close the loop — send the reviewer back to where the campaign lives. */}
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-surface p-4">
            <p className="text-sm text-muted">Happy with the delivery? Approve it and wrap up the campaign.</p>
            <div className="flex gap-2">
              <Link href={`/deals/${deal.id}/performance`} className="rounded-lg border border-border px-4 py-2 text-sm font-medium hover:bg-background">
                View performance
              </Link>
              <Link href="/campaigns" className="rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white hover:bg-brand-dark">
                Approve &amp; back to Campaigns
              </Link>
            </div>
          </div>
        </div>
